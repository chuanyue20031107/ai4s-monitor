...
- `.github/workflows/crawl-and-deploy.yml` and `scripts/crawl.mjs` are unchanged: cron `17 */6 * * *`, default batch size 36, existing publication behavior, robots/DNS/redirect safeguards, and WeRSS configuration remain intact. Manual operations restore the exact scheduled cursor/settings bytes and merge only the targeted source runtime records into the full inventory.
